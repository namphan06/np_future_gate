...
class CV2InputScreen extends StatefulWidget {
  ...
  const CV2InputScreen({Key? key, this.cvId}) : super(key: key);

  @override
  State<CV2InputScreen> createState() => _CV2InputScreenState();
}

class _CV2InputScreenState extends State<CV2InputScreen> {
  ...
  Future<void> _initializeData() async {
    if (widget.cvId != null) {
      ...
      try {
        ...
        setState(() => _cvData = data ?? _getEmptyCV2Data());
      } catch (e) {
        _showError('Không thể tải dữ liệu CV: $e');
        setState(() => _cvData = _getEmptyCV2Data());
      } finally {
        setState(() => _isLoading = false);
      }
    } else {
      _cvData = _getEmptyCV2Data();
    }
  }

  Map<String, dynamic> _getEmptyCV2Data() => {
    'mcv': 'CV002',
  ...
  void _onSectionTap(String section) {
    ...
    Navigator.of(context).push(MaterialPageRoute(builder: (ctx) {
      return Scaffold(
        ...
        body: CV1InputForm( // Reusing generic form
          section: section,
          data: _cvData,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.cvId != null ? 'Chỉnh sửa CV (Mẫu 2)' : 'Tạo CV mới (Mẫu 2)'),
        actions: [
          IconButton(
