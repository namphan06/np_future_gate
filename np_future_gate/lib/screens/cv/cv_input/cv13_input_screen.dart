...
class CV13InputScreen extends StatefulWidget {
  const CV13InputScreen({super.key, this.cvId});
  final String? cvId;
...
class _CV13InputScreenState extends State<CV13InputScreen> {
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: SingleChildScrollView(controller: _scrollController, padding: const EdgeInsets.all(24),
          child: Center(child: Container(constraints: const BoxConstraints(maxWidth: 900), decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.1), blurRadius: 10, offset: const Offset(0, 4))]),
            child: Cv13(data: _cvData, onSectionTap: _onSectionTap),
          )),
        ),
      ),
    );
