...
class CV16InputScreen extends StatefulWidget {
  const CV16InputScreen({super.key, this.cvId});
  final String? cvId;
...
class _CV16InputScreenState extends State<CV16InputScreen> {
  ...
  Map<String, dynamic> _getEmptyData() => {
    'mcv': 'CV016', 'type': 'field',
    'personal_info': {'full_name': '', 'title': '', 'email': '', 'phone': '', 'address': '', 'avatar_url': ''},
  ...
  String _sectionTitle(String section) {
    switch (section) {
      ...
      case 'summary': return 'Tóm tắt chuyên môn';
      case 'experiences': return 'Kinh nghiệm pháp lý';
      case 'education': return 'Học vấn & Hành nghề';
      case 'skills': return 'Lĩnh vực chuyên môn';
      case 'certifications': return 'Chứng chỉ';
      case 'projects': return 'Vụ việc tiêu biểu';
      ...
      body: Container(
        ...
        child: SingleChildScrollView(controller: _scrollController, padding: const EdgeInsets.all(24),
          child: Center(child: Container(constraints: const BoxConstraints(maxWidth: 800), decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.1), blurRadius: 10, offset: const Offset(0, 4))]),
            child: Cv16(data: _cvData, onSectionTap: _onSectionTap),
          )),
        ),
      ),
    );
