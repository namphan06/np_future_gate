...
/// CV10 Input Screen - Elegant Minimalist
class CV10InputScreen extends StatefulWidget {
  const CV10InputScreen({super.key, this.cvId});
  final String? cvId;

  @override
  State<CV10InputScreen> createState() => _CV10InputScreenState();
}

class _CV10InputScreenState extends State<CV10InputScreen> {
  ...
  Map<String, dynamic> _getEmptyData() => {
    ...
    'type': 'general',
    'personal_info': {
      'full_name': '', 'title': '', 'email': '', 'phone': '', 'address': '', 'avatar_url': '',
    },
    'summary': '',
    'experiences': [],
    'education': [],
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(appBar: AppBar(title: const Text('Đang tải...')), body: const Center(child: CircularProgressIndicator()));
    }
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.cvId != null ? 'Chỉnh sửa CV' : 'Tạo CV mới'),
        actions: [IconButton(icon: const Icon(Icons.save), onPressed: _saveCV, tooltip: 'Lưu CV')],
      ),
      body: Container(
        ...
        child: SingleChildScrollView(
          ...
          child: Center(
            child: Container(
              ...
              child: Cv10(data: _cvData, onSectionTap: _onSectionTap),
            ),
          ),
        ),
      ),
