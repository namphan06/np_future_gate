...
class CV15InputScreen extends StatefulWidget {
  final String? cvId;
  const CV15InputScreen({super.key, this.cvId});

  @override
  State<CV15InputScreen> createState() => _CV15InputScreenState();
}

class _CV15InputScreenState extends State<CV15InputScreen> {
  ...
  String _sectionTitle(String section) {
    switch (section) {
      ...
      case 'summary': return 'Triết lý giáo dục';
      case 'experiences': return 'Kinh nghiệm giảng dạy';
      case 'education': return 'Học vấn & Bằng cấp';
      case 'skills': return 'Kỹ năng';
      case 'certifications': return 'Chứng chỉ';
      case 'projects': return 'Nghiên cứu & Công bố';
      ...
      body: Container(
        ...
        child: SingleChildScrollView(controller: _scrollController, padding: const EdgeInsets.all(24),
          child: Center(child: Container(constraints: const BoxConstraints(maxWidth: 800), decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 10, offset: const Offset(0, 4))]),
            child: Cv15(data: _cvData, onSectionTap: _onSectionTap),
          )),
        ),
      ),
    );
